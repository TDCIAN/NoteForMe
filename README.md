...
1. 싱글톤 패턴을 자바로 구현한 실행 결과 (0점)
- 답안: 3

2. MAC 주소를 가로채는 공격: (    ) Spoofing (0점)
- 답안: ARP
- 출제: 소프트웨어 개발 보안 구축 1강

3. 데이터 제어어(DCL) 중 GRANT 에 대한 개념 서술 (5점)
- 답안: 사용자에게 권한 부여
- 출제: SQL 응용 1강

4. AAA 서버의 각 A에 해당 되는 개념을 풀어써주고 맞는 용어를 매핑 시키는 문제 (5점)
...
5. 디자인 패턴 (5점)
- 답안: Factory Method
- 출제: 애플리케이션 테스트 관리 3강, 서버프로그램 구현 2강

6. 결합도(coupling) 종류 중 단순 처리할 대상인 값만 전달되는 게 아니라 어떻게 처리를 해야 한다는 제어 요소가 전달되는 경우이다 (5점)
- 답안: Control
- 출제: 서버프로그램 구현 2강
...
8. 하위 모듈에서 시작한다는 ( ) 테스트로, 하위 모듈들을 포괄하는 ( )이 있어야 함. (5점)
보기가 있는 문제.
- 답안: 상향식 통합, 드라이버
- 출제: 애플리케이션 테스트 관리 2강
...
10. SQL Cross Join Count (5점)
- 답안: 4
- 출제: SQL 응용 2강

11. 데이터 접근 방식. <값, 주소>. 한 단어로 쓰시오 (5점)
- 답안: Index(또는 인덱스)
- 출제: 데이터 입출력 구현 2강

12. 테스트 케이스 관련 표를 보여주고 컬럼 제목란에 해당 되는 내용 매핑하기 (3점 ? 5점)
- 답안: 테스트 조건, 테스트 데이터, 예상 결과
- 출제: 애플리케이션 테스트 관리 1강

13. UML 종류 중 ( ) 다이어그램 설명해 주고 빈칸 넣기. 속성, 메소드 포함됨 (5점)
- 답안: 클래스
- 출제: 요구사항 확인 1강

14. OSI 7 Layer 중 3개 계층 설명 후 해당하는 계층 (3점 혹은 5점)
- 답안: 데이터 링크, 네트워크, 프레젠테이션
- 출제: 응용 SW 기초 기출 활용 2강

15. 암호화 기법, 64비트, IBM, 16라운드 (0점)
- 답안: DES
...
17. 자바 조건문 처리 후 a 출력 (0점)
- 답안: 7
- 출제: 프로그래밍 언어 활용

18. 입력 자료 간의 관계와 출력에 영향을 미치는 상황을 체계적으로 분석 후 효용성이 높은 테스트 케이스를 선정해서 테스트하는 기법 (5점)
- 답안: Cause Effect Graph
- 출제: 애플리케이션 테스트 관리 1강

19. 사용자가 키보드, 마우스 등을 이용...  인터페이스. 유저 인터페이스로 윈도우, 맥킨토시 등이 있다. (5점)
- 답안: GUI
- 출제: 화면 설계 1강
